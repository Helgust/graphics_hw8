...
void SimpleRender::SetupSimplePipeline()
{
  ...
  shader_paths[VK_SHADER_STAGE_FRAGMENT_BIT] = YAN_FRAGMENT_SHADER_PATH + ".spv";
  shader_paths[VK_SHADER_STAGE_VERTEX_BIT]   = VERTEX_SHADER_PATH + ".spv";
  shader_paths[VK_SHADER_STAGE_GEOMETRY_BIT]   = GEOMETRY_SHADER_PATH + ".spv";

  maker.LoadShaders(m_device, shader_paths);

  m_normalPipeline.layout = maker.MakeLayout(m_device, {}, sizeof(pushConst2M));
  maker.SetDefaultState(m_width, m_height);

  m_normalPipeline.pipeline = maker.MakePipeline(m_device, m_pScnMgr->GetPipelineVertexInputStateCreateInfo(),
                                                       m_screenRenderPass, {VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR});
}
                                            ...
                                            VkImageView, VkPipeline a_pipeline)
{
  ...
  ///// draw final scene to screen
  {
    ...
    for (uint32_t i = 0; i < m_pScnMgr->InstancesNum(); ++i)
    {
     ...
     vkCmdBindPipeline(a_cmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, m_normalPipeline.pipeline);
    ...
    for (uint32_t i = 0; i < m_pScnMgr->InstancesNum(); ++i)
    {
      ...
      pushConst2M.model = m_pScnMgr->GetInstanceMatrix(i);
      vkCmdPushConstants(a_cmdBuff, m_normalPipeline.layout, stageFlags, 0,
                         sizeof(pushConst2M), &pushConst2M);
...
void SimpleRender::Cleanup()
{
  ...
  if (m_normalPipeline.pipeline != VK_NULL_HANDLE)
  {
    vkDestroyPipeline(m_device, m_normalPipeline.pipeline, nullptr);
    m_basicForwardPipeline.pipeline = VK_NULL_HANDLE;
  }
  if (m_normalPipeline.layout != VK_NULL_HANDLE)
  {
    vkDestroyPipelineLayout(m_device, m_normalPipeline.layout, nullptr);
    m_basicForwardPipeline.layout = VK_NULL_HANDLE;
  }
